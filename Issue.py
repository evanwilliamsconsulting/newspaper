...
class IssueList(BrowserView):
    """ A List of the Issues """
    def __init__(self,context,request):
	""" Initialize context and request """
...
class Issue(BaseFolder,BrowserDefaultMixin):
    ...
    security.declarePublic(permissions.View,'the_issues')
    def listItems(self):
	"""
	hey
	"""
    ...
    def listPages(self):
	"""
	So much easier that PHP to add just a function to a class.
	Must be because the class definitions have a different foundation
	"""
	items = self.listItems()
	pages = []
	for item in items:
	     pages.append(item.getId())
	return pages
	...
	parent = self.aq_inner
        return showTemplate.pt_render(REQUEST,parent)
    ...
    def pdf(self,REQUEST): 
        ...
        pdf
        """
	theTemplate = PDFPageTemplate('issue','/opt/newhol/press/products/Newspaper/skins/newspaper_templates/issue.pd')
	parent = self.aq_inner
    ...
    def webfetch(self,REQUEST): 
        ...
        web 
        """
	theTemplate = PDFPageTemplate('issue','/opt/development/newholland/press/products/Newspaper/skins/newspaper_templates/issue.pd')
	parent = self.aq_inner
