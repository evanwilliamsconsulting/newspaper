...
BookContents_schema = BaseSchema.copy() + \
    schema.copy()

##code-section after-schema #fill in your manual code here
##/code-section after-schema

class BookContents(BaseContent, BrowserDefaultMixin):
    """
    """
    security = ClassSecurityInfo()

    implements(interfaces.IBookContents)

    meta_type = 'BookContents'
    _at_rename_after_creation = True

    schema = BookContents_schema
    ...
    def applyBookContentsToIssue(self):
	    """
	    Test
	    """
    	    skin = self.portal_skins.newspaper_templates
    ...
    def applyBookContentsToColumn(self):
	    """
	    Test
	    """
    	    skin = self.portal_skins.newspaper_templates
    ...
    def drawALine(self,c,x,y,theLine):
	    ...
	    totalWordLength = 0
	    for word in words:
		totalWordLength += c.stringWidth(word,"Times-Roman",12)
	    ...
	    for word in words:
		stringWidth = c.stringWidth(word,"Times-Roman",12)
	        textobject.setTextOrigin(x,y)
    ...
    def callPDFPDTBySameName(self,c,x,y,REQUEST,parent,top,pagenumber):
	    ...
	    Test
	    """
	    theparent = parent.aq_inner.aq_parent
	    topparent = theparent.aq_inner.aq_parent
	    containers = topparent.listFolderContents(contentFilter={"portal_type":"Page"})
	    print self.Title()
	    xorig = x
	    yorig = y
	    xleft = self.getLeft()
	    ...
	    self.drawALine(c,x,y,line)
	    y += 20
	    resultString = "Table of Contents:"
	    startPage = self.getStartPage()
	    endPage = self.getEndPage()
	    i = 0
	    k = 1
	    for container in containers:
		j = 0
		...
		for item in items:
			if j == 1:
				...
				if iterator == 1  or iterator ==  2:
					...
					k += 1
					if j == 1:
						resultString = resultString + "......................." + str(page)
	    	    				self.drawALine(c,x,y,resultString)
						j = 0
		    				y += 12
				j += 1
		i += 1
		...
		for item in items:
			if j == 1:
				resultString = item.title
	        	if i>=startPage - 8 and  i<=endPage - 10:	
				iterator = i % 4
    ...
    def publish(self,REQUEST):
	    ...
	    showTemplate=skin.publishBookContents
	    return showTemplate()

registerType(BookContents, PROJECTNAME)
# end of class BookContents
