...
class Invoice(BaseFolder):
	...
	dateOfInvoice = self.getDateOfInvoice()
	theDate=((str(dateOfInvoice)).split(' '))[0]
	...
	for aLine in self.listFolderContents():
		...
		total = extension * (100-discount) / 100
    ...
    def letter(self,REQUEST): 
        ...
        showTemplate=skin.showIssue
	output = StringIO()
	c = canvas.Canvas(output,pagesize=letter,bottomup=1)
	x=35
	y=50
	#self.renderMasthead(c,x,y)
        textobject = c.beginText()
